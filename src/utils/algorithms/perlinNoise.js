 ...
 */
export function computeEndPoints(col, row, scale, x_change, y_change) {
    let perlin_noise = perlin2(y_change, x_change);
    let angle = perlin_noise * 2 * Math.PI;
    let x_end = scale * (col + Math.cos(angle));
    let y_end = scale * (row + Math.sin(angle));
